...
<td class="filename"><?php if ($p_media !== null) : ?><?php if ($p_media->getMediaStatus($this->getRequest()) === 'banned') : ?>File banned
...
<a href="<?= ($p_media->getMediaLink($this->getRequest())) ? $p_media->getMediaLink($this->getRequest()) : $p_media->getRemoteMediaLink($this->getRequest()) ?>" target="_blank">
    <?php if (mb_strlen($p_media->getMediaFilenameProcessed()) > 30) : ?>
        ...
        <?= mb_substr($p_media->getMediaFilenameProcessed(), 0, 24, 'utf-8') . ' (...)' . mb_substr($p_media->getMediaFilenameProcessed(), mb_strrpos($p_media->getMediaFilenameProcessed(), '.', 'utf-8'), null, 'utf-8') ?></span></a><a href="<?= $p_media->getMediaDownloadLink($this->getRequest()) ?>" download="<?= $p_media->getMediaFilenameProcessed() ?>" class="btnr parent pull-right"><i class="icon-download-alt"></i></a>
    <?php else: ?>
     <?= $p_media->getMediaFilenameProcessed() ?></a><a href="<?= $p_media->getMediaDownloadLink($this->getRequest()) ?>" download="<?= $p_media->getMediaFilenameProcessed() ?>" class="btnr parent pull-right"><i class="icon-download-alt"></i></a>
	<?php endif; ?>
<?php endif; endif; ?>
</td>
<td class="tag">
<?php if (substr($p_media->getMediaFilenameProcessed(), -3) == "swf") : 
	if ($p_media->exif) :
	$exiftemp = json_decode($p_media->exif) ?><span class="post_file_tag" rel="tooltip" title="Search for files with this tag"><a href="<?= $this->uri->create($radix->shortname . '/search/tag/' . $exiftemp->{'Tag'}) ?>"><?= $exiftemp->{'Tag'} ?></a></span><?php
	else: ?><span class="post_file_tag" rel="tooltip" title="This tag is empty for some reason">Empty</span>
	<?php endif; ?>
<?php endif; ?>
</td>
<td class="subject">
...
<td class="thread">
<a href="<?= $this->uri->create($radix->shortname . '/thread/' . $p->num) ?>" class="btnr parent"><?= _i('Reply') ?></a><?= (isset($p->count_all) && $p->count_all > 50) ? '<a href="' . $this->uri->create($radix->shortname . '/last/50/' . $p->num) . '" class="btnr parent">' . _i('Last 50') . '</a>' : '' ?><?php if ($radix->archive == 1) : ?><a href="http://boards.4chan.org/<?= $radix->shortname . '/thread/' . $p->num ?>" class="btnr parent"><?= _i('Original') ?></a><?php endif; ?><a href="<?= $this->uri->create($radix->shortname . '/report/' . $p->doc_id) ?>" class="btnr parent" data-function="report" data-post="<?= $p->doc_id ?>" data-post-id="<?= $p->num ?>" data-board="<?= htmlspecialchars($p->radix->shortname) ?>" data-controls-modal="post_tools_modal" data-backdrop="true" data-keyboard="true"><?= _i('Report') ?></a><?php if ($this->getAuth()->hasAccess('maccess.mod')) : ?><a href="<?= $this->uri->create($radix->shortname . '/delete/' . $p->doc_id) ?>" class="btnr parent" data-function="delete" data-post="<?= $p->doc_id ?>" data-post-id="<?= $p->num ?>" data-board="<?= htmlspecialchars($p->radix->shortname) ?>" data-controls-modal="post_tools_modal" data-backdrop="true" data-keyboard="true"><?= _i('Delete') ?></a><?php endif; ?>
</td>
</tr>
        <?php
        endforeach;
